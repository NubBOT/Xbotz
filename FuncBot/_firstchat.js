...
let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? wann.user.jid : m.sender
let username = wann.getName(who)
let logo = await (await fetch(`${logos()}`)).buffer()
let tag = `@${m.sender.split('@')[0]}`
 m, { contextInfo: { mentionedJid: wann.parseMention(tag) }}
    let waofc = `@${'0'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: wann.parseMention(tag) }}
 let ow = `@${'6285870824804'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: wann.parseMention(tag) }}
    ...
    let tks = `Saya adalah Fss BOT salah satu Bot Whatsapp yang diciptakan oleh : ${ow}
Silahkan tekan Tombol ${user.registered ? '"Menu"' : '"Verify"'} untuk ${user.registered ? 'melihat list command dari' : 'mendaftar sebelum menggunakan'} FSS BOT.
